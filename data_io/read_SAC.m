...
% read_SAC  Reads Z-component SAC files and assembles a DataStruct array.
%
% Usage:
...
%
% Inputs:
%   dataFolder : Folder path containing SAC files
%   maxFiles   : (optional) Maximum number of files to read (default = Inf)
%
% Output:
%   DataStruct : Structure array, each element contains E/N/Z waveforms
%                and the corresponding station, event, and time-axis info.
% Author: Yunfeng Chen (Refined by ChatGPT)
% Date:   Jan. 25, 2025
    %--------------- 0. Check inputs & default parameters ---------------%
    if nargin < 2
        maxFiles = Inf; % If not specified, read all files
    end

    %--------------- 1. Retrieve SAC file list (Z-component) ---------------%
    sacfiles = dir(fullfile(dataFolder, '*Z.*.SAC')); 
    nAllFiles = length(sacfiles);          % Total number of matched Z files
    nFiles    = min(nAllFiles, maxFiles);  % Do not exceed maxFiles

    % If there are no matching files, return immediately
    if nFiles == 0
        DataStruct = [];
        warning('No Z-component SAC files found in: %s', dataFolder);
        return;
    end

    %--------------- 2. Pre-allocate DataStruct ---------------%
    % Create a template struct using initEmptyStruct(), then replicate nFiles times.
    DataStruct(1:nFiles) = initEmptyStruct();
    % Use idx to record the actual number of valid records
    idx = 0;

    %--------------- 3. Read files and fill DataStruct ---------------%
    for i = 1:nFiles
        % Print progress for every 10% of the processed files
        if mod(i, max(round(nFiles/10), 1)) == 0
            fprintf('Reading file %d / %d\n', i, nFiles);
        end

        %=== 3.1 Full path to the Z file and read it ===%
        Zfile = fullfile(sacfiles(i).folder, sacfiles(i).name);
        [tZ, dataZ, hdrZ] = fget_sac(Zfile);

        %=== 3.2 Infer the corresponding N/E filenames ===%
        ...
        %=== 3.3 Check if N/E files exist; skip if missing ===%
        if ~isfile(Nfile) || ~isfile(Efile) || all(dataZ == 0)
        ...
        %=== 3.5 Increment valid counter and construct a local tmpStruct ===%
        idx = idx + 1;
        ...
        %=== 3.6 Write results into the pre-allocated DataStruct ===%
        DataStruct(idx) = tmpStruct;
    end

    %--------------- 4. Truncate DataStruct to the valid records ---------------%
    % Some files may have been skipped, so only idx entries are valid
    DataStruct = DataStruct(1:idx);

    % If idx=0, then no valid data was read
    if idx == 0
        warning('No valid 3-component data read from %s (processed %d files).', dataFolder, nFiles);
    end
end
