...
minSNR = 2;    % Minimum SNR of the RF required for CCP imaging
...
for iEvent = 6
    ...
    if DeconvParam.radonfilter
        ...
        gather = structureOrientedFilter(gather, RadonParam);
        for k = 1:length(gather)
            gather(k).Waveforms.dataProcessed = gather(k).Waveforms.dataStructureOrientedFiltered;
        end
