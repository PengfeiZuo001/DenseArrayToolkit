...
nx = gridStruct.nx;
ny = gridStruct.ny;
vp3d = reshape(v1,nz,nx,ny);
vs3d = reshape(v2,nz,nx,ny);
...
switch ModelType
    case '1D'
        for iz = 1:nz
            tmp1(iz,1) = mean(vp3d(iz,:,:),'all');
            tmp2(iz,1) = mean(vs3d(iz,:,:),'all');

            v11 = moving_avg(tmp1, N, 'constant', 1);
            v22 = moving_avg(tmp2, N, 'constant', 1);

            gridStruct.vp = repmat(v11(:), 1, nx, ny);
            gridStruct.vs = repmat(v22(:), 1, nx, ny);
        end
    ...
    case '2D'
        ...
        for iz = 1:nz
            for ix = 1:nx
                tmp1(iz,ix) = mean(vp3d(iz,ix,:),'all');
                tmp2(iz,ix) = mean(vs3d(iz,ix,:),'all');
            end
        end
        v11 = moving_avg(tmp1, N, 'constant', 1);
        v11 = moving_avg(v11, N, 'constant', 2);
        
        v22 = moving_avg(tmp2, N, 'constant', 1);
        v22 = moving_avg(v22, N, 'constant', 2);

        gridStruct.vp = repmat(v11, 1, 1, ny);
        gridStruct.vs = repmat(v22, 1, 1, ny);

    case '3D'
        win = [N N N];   
        Vp_smooth = smooth3(vp3d, 'box', win);
        Vs_smooth = smooth3(vs3d, 'box', win);
        
        gridStruct.vp = Vp_smooth;
        gridStruct.vs = Vs_smooth;
end
end
